# Napkin Runbook

## Curation Rules
- Re-prioritize on every read.
- Keep recurring, high-value notes only.
- Max 10 items per category.
- Each item includes date + "Do instead".

## Execution & Validation (Highest Priority)
1. **[2026-08-10] Preserve the published portfolio until replacement is verified**
   Do instead: build and validate the Astro output locally before committing or publishing.
2. **[2026-08-10] Animation must be progressive enhancement**
   Do instead: scope hidden reveal states under the root `.js` class so static HTML stays visible without JavaScript.

## Shell & Command Reliability
1. **[2026-08-10] Use native PowerShell for Windows file operations**
   Do instead: resolve exact paths first and use `Copy-Item` or `Remove-Item -LiteralPath` end to end.

## Domain Behavior Guardrails
1. **[2026-08-10] Portfolio claims must remain verifiable**
   Do instead: use CV-backed cases and clearly separated service pages, never fictional delivered projects.
2. **[2026-08-10] Social previews need a raster default**
   Do instead: use the 1200×630 PNG for Open Graph while project SVGs remain available to page content and structured data.

## User Directives
1. **[2026-08-10] Signal Room is the portfolio's visual direction**
   Do instead: use deep navy, cyan, electric blue, real photography, and operational UI motifs; never use orange.
2. **[2026-08-10] Impeccable is mandatory from design through final QA**
   Do instead: maintain PRODUCT.md and DESIGN.md, then run responsive, accessibility, critique, audit, and polish passes.
3. **[2026-08-10] English is canonical and Portuguese uses pt-BR**
   Do instead: mirror public content under `/pt/` and emit canonical plus hreflang metadata.
